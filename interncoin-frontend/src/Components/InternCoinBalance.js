...
const InternCoinBalance = () => {
  const { account, connectWallet, logout, balance, setBalance } = useContext(WalletContext);
  ...
  const fetchBalance = useCallback(async () => {
    try {
      ...
      setBalance(formattedBalance); // Correctly update balance
    } catch (error) {
  ...
  return (
    <div>
      {account ? (
        <>
          <div className="connected-badge">Connected: {account}</div>
          <button onClick={logout} className="logout-button">Log Out</button>
        </>
      ) : (
        <div className="not-connected-badge">Wallet Not Connected</div>
      )}
